...
export class ComponentHook {
  ...
  async beforeDestroy(
    ...
    const { componentControlerIbfk3, componentIbfk2, routerIbfk3 } = await Bb.props({
        componentControlerIbfk3: ComponentControlerModel.findOne({
          where: {
            [COMPONENT_CONTROLER.COMPONENT_ID]: model.get('id'),
          },
        }),
        componentIbfk2: ComponentModel.findOne({
          where: {
            [COMPONENT.PARENT_ID]: model.get('id'),
          },
        }),
        routerIbfk3: RouterModel.findOne({
          where: {
            [ROUTER.COMPONENT_ID]: model.get('id'),
          },
        }),
    });
    if (componentControlerIbfk3 || componentIbfk2 || routerIbfk3) {
