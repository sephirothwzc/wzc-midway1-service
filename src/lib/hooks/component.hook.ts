...
export class ComponentHook {
  ...
  async beforeUpdate(
    ...
    if (
      changed.includes(COMPONENT.COMPONENT_CODE) &&
      model.get('ComponentCode')
    ) {
      ...
      if (item0) {
        throw new Error('组件名称[unique]已存在');
      }
    }

    if (
      changed.includes(COMPONENT.COMPONENT_KEY) &&
      model.get('ComponentKey')
    ) {
      ...
      if (item1) {
        throw new Error('组件唯一约束key[unique]已存在');
      }
    }

    if (
      changed.includes(COMPONENT.COMPONENT_NAME) &&
