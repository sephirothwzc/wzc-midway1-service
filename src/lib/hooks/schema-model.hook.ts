...
export class SchemaModelHook {
  ...
  async beforeDestroy(
    ...
    const { businessSchemaIbfk4, schemaModelIbfk2, schemaModelRoleIbfk3 } = await Bb.props({
        businessSchemaIbfk4: BusinessSchemaModel.findOne({
          where: {
            [BUSINESS_SCHEMA.SCHEMA_MODEL_ID]: model.get('id'),
          },
        }),
        schemaModelIbfk2: SchemaModelModel.findOne({
          where: {
            [SCHEMA_MODEL.PARENT_ID]: model.get('id'),
          },
        }),
        schemaModelRoleIbfk3: SchemaModelRoleModel.findOne({
          where: {
            [SCHEMA_MODEL_ROLE.SCHEMA_MODEL_ID]: model.get('id'),
          },
        }),
    });
    if (businessSchemaIbfk4 || schemaModelIbfk2 || schemaModelRoleIbfk3) {
