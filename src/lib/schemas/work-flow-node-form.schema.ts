...
// #region Graphql
export const workFlowNodeFormMutationCreate = Joi.object().keys({
  ...
  flowNodeId: Joi.string().allow('').allow(null).description('工作流节点id'),
...
export const workFlowNodeFormMutationUpdate = Joi.object().keys({
id: Joi.string().allow(''),
  businessCode: Joi.string().allow('').allow(null).description('业务编码权限用'),
  createdAt: Joi.date().allow(null).description('创建时间'),
  createdId: Joi.string().allow('').allow(null).description('创建人id'),
...
export const workFlowNodeFormBulkMutation = Joi.array().items(workFlowNodeFormMutationCreate);
// #endregion
