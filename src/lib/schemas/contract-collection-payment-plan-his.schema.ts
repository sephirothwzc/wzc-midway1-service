...
export const contractCollectionPaymentPlanHisMutationCreate = Joi.object().keys({
  ...
  collectedRemark: Joi.string().allow('').allow(null).description('预算来源文件号'),
  collectedTime: Joi.date().allow(null).description('收款时间'),
  contractCollectionPaymentPlanId: Joi.string().allow('').allow(null).description('合同收款计划id'),
...
export const contractCollectionPaymentPlanHisBulkMutation = Joi.array().items(contractCollectionPaymentPlanHisMutationCreate);
// #endregion
