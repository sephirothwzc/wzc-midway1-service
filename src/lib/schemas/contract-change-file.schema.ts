...
// #region Graphql
export const contractChangeFileMutationCreate = Joi.object().keys({
  businessCode: Joi.string()
    .allow('')
    .allow(null)
    .description('业务编码权限用'),
  contractChangeId: Joi.string().allow('').allow(null).description('变更id'),
...
export const contractChangeFileMutationUpdate = Joi.object().keys({
  id: Joi.string().allow(''),
  businessCode: Joi.string()
    .allow('')
    .allow(null)
...
export const contractChangeFileBulkMutation = Joi.array().items(
  contractChangeFileMutationCreate
);
// #endregion
