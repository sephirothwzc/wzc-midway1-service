...
// #region Graphql
export const projectChangeFileMutationCreate = Joi.object().keys({
  ...
  projectChangeId: Joi.string().allow('').allow(null).description('变更id'),
...
export const projectChangeFileMutationUpdate = Joi.object().keys({
id: Joi.string().allow(''),
  businessCode: Joi.string().allow('').allow(null).description('业务编码权限用'),
  createdAt: Joi.date().allow(null).description('创建时间'),
  createdId: Joi.string().allow('').allow(null).description('创建人id'),
...
export const projectChangeFileBulkMutation = Joi.array().items(projectChangeFileMutationCreate);
// #endregion
