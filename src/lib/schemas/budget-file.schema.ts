...
// #region Graphql
export const budgetFileMutationCreate = Joi.object().keys({
...
export const budgetFileMutationUpdate = Joi.object().keys({
id: Joi.string().allow(''),
  budgetId: Joi.string().allow('').allow(null).description('预算id'),
  businessCode: Joi.string().allow('').allow(null).description('业务编码权限用'),
  createdAt: Joi.date().allow(null).description('创建时间'),
...
export const budgetFileBulkMutation = Joi.array().items(budgetFileMutationCreate);
// #endregion
