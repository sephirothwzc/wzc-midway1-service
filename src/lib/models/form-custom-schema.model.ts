...
export class FormCustomSchemaModel extends BaseModel {
  ...
  @HasMany(() => SchemaOrmModel, 'form_custom_schema_id')
  schemaOrmFormCustomSchemaId: Array<SchemaOrmModel>;

  @HasMany(() => WorkFlowNodeFormModel, 'form_custom_schema_id')
  workFlowNodeFormFormCustomSchemaId: Array<WorkFlowNodeFormModel>;

  @HasMany(() => WorkFlowOrmModel, 'form_custom_schema_id')
  workFlowOrmFormCustomSchemaId: Array<WorkFlowOrmModel>;
...
export const createOptions = () => {
  return (
    param: FormCustomSchemaModel
  ): { include?: [any]; transaction?: any; validate?: boolean } => {
    if (!param.schemaOrmFormCustomSchemaId && !param.workFlowNodeFormFormCustomSchemaId && !param.workFlowOrmFormCustomSchemaId) {
      return {};
    }
    const include: any = [];
    param.schemaOrmFormCustomSchemaId &&
      param.schemaOrmFormCustomSchemaId.length > 0 &&
      include.push({ model: SchemaOrmModel, as: 'schemaOrmFormCustomSchemaId' });
    param.workFlowNodeFormFormCustomSchemaId &&
      param.workFlowNodeFormFormCustomSchemaId.length > 0 &&
      include.push({ model: WorkFlowNodeFormModel, as: 'workFlowNodeFormFormCustomSchemaId' });
    param.workFlowOrmFormCustomSchemaId &&
      param.workFlowOrmFormCustomSchemaId.length > 0 &&
      include.push({ model: WorkFlowOrmModel, as: 'workFlowOrmFormCustomSchemaId' });
    return { include };
  };
};
providerWrapper([
  {
    id: 'formCustomSchemaModel.createOptions',
